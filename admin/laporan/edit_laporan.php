...
if (isset($_POST['submit'])) {
    $judul = $_POST['judul'];
    $id_jenis = $_POST['identitas'];
    $lokasi = $_POST['lokasi'];
    $tgl = $_POST['tgl'];
    $mytextarea = $_POST['mytextarea'];
    $created_at = date('Y-m-d');
    mysqli_query($conn,"UPDATE laporan_kegiatan SET judul_laporan='$judul',identitas='$id_jenis',lokasi='$lokasi',tgl='$tgl',isi='$mytextarea',created_at='$created_at',status='PENDING' WHERE id_laporan='$id_laporan'");
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-9">
                                ...
                                while ($jenis = mysqli_fetch_array($query_jenis)) { ?>
									<option value="<?= $jenis['identitas'] ?>" <?= $jenis['identitas']==$data_laporan['identitas'] ? ' selected' : '' ?>><?= ucwords($jenis['judul']) ?></option>
                                <?php
                                };
                                ?>
								</select>
